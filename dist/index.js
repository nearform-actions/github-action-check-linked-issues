...
// src/constants.js
var ERROR_MESSAGE = "No linked issues found. Please add the corresponding issues in the pull request description.";
...
function shouldRun() {
  const excludeBranches = parseCSV(core.getInput("exclude-branches", {
    required: false
  }));
  if (!excludeBranches.length)
    return true;
  const sourceBranch = github.context.payload.pull_request.head.ref;
  const result = excludeBranches.some((p) => (0, import_minimatch.default)(sourceBranch, p));
  if (result) {
...
function filterLinkedIssuesComments(issues = []) {
  return issues.filter((issue) => {
    var _a;
    const match = (_a = issue == null ? void 0 : issue.body) == null ? void 0 : _a.match(/<!-- metadata = (.*) -->/);
    if (match) {
...
async function getPrComments({
  ...
  const issues = await octokit.paginate("GET /repos/{owner}/{repo}/issues/{prNumber}/comments", {
    owner: repoOwner,
    repo: repoName,
    prNumber
  });
...
function deleteLinkedIssueComments(octokit, comments) {
  return Promise.all(comments.map(({ node_id }) => octokit.graphql(`
...
// src/action.js
var format = (obj) => JSON.stringify(obj, void 0, 2);
async function run() {
  var _a, _b;
  core2.info(`
    *** ACTION RUN - START ***
    `);
  try {
    const { payload, eventName } = github2.context;
    if (eventName !== "pull_request_target" && eventName !== "pull_request") {
      throw new Error(`This action can only run on "pull_request_target" or "pull_request", but "${eventName}" was received. Please check your workflow.`);
    }
    core2.debug(`
    *** PAYLOAD ***
    ${format(payload)}
    ...
    *** GRAPHQL DATA ***
    ${format(data)}
    `);
    const pullRequest = (_a = data == null ? void 0 : data.repository) == null ? void 0 : _a.pullRequest;
    const linkedIssuesCount = (_b = pullRequest == null ? void 0 : pullRequest.closingIssuesReferences) == null ? void 0 : _b.totalCount;
    ...
    core2.setOutput("linked_issues_count", linkedIssuesCount);
    if (linkedIssuesComments.length) {
      await deleteLinkedIssueComments(octokit, linkedIssuesComments);
      core2.debug(`${linkedIssuesComments.length} Comment(s) deleted.`);
    }
    if (!linkedIssuesCount) {
      const prId = pullRequest == null ? void 0 : pullRequest.id;
      const shouldComment = core2.getInput("comment") && prId;
      if (shouldComment) {
        const body = core2.getInput("custom-body-comment");
        await addComment({ octokit, prId, body });
        core2.debug("Comment added");
      }
      core2.setFailed(ERROR_MESSAGE);
    }
  } catch (error) {
